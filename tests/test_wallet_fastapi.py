...
def test_create_wallet_success(client: TestClient) -> None:
    ...
    response = client.post("/wallets", json=user_response.json()["user"])
...
def test_create_wallet_user_not_found(client: TestClient) -> None:
    request = {"private_key": str(uuid.uuid4())}

    response = client.post("/wallets", json=request)
...
def test_create_wallet_wallet_limit_reached(client: TestClient) -> None:
    ...
    for i in range(MAX_WALLETS_PER_USER):
        client.post("/wallets", json=user_response.json()["user"])
...
def test_get_wallet_by_address_success(client: TestClient) -> None:
    ...
    wallet_response = client.post("/wallets", json=user_response.json()["user"])
...
def test_get_wallet_by_address_wallet_not_found(client: TestClient) -> None:
    ...
    assert response.json() == {
        "error": {
            "message": "Wallet with address <00000000-0000-0000-0000-000000000001> does not exist."
        }
    }
...
def test_get_wallet_by_address_invalid_owner(client: TestClient) -> None:
    ...
    wallet_response = client.post("/wallets", json=user_response1.json()["user"])
    ...
    assert response.json() == {
        "error": {
            "message": f"Wallet with address <{str(public_key)}> does not belong to the correct owner."
        }
    }
...
def test_get_wallet_transactions_wallet_not_found(client: TestClient) -> None:
    ...
    assert response.status_code == 405
    assert response.json() == {
        "error": {"message": "Wallet with address <1> does not exist."}
    }
