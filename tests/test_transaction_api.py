...
def test_make_transaction_success(client: TestClient) -> None:
    ...
    response = client.post("/transactions", json=request_data, headers=user_response.json()["user"])
...
def test_make_transaction_user_does_not_exist(client: TestClient) -> None:
    request_data = {"from_key": str(uuid.uuid4()), "to_key": str(uuid.uuid4()), "amount": 100}
    response = client.post("/transactions", json=request_data, headers={"api_key": str(uuid.uuid4())})
...
def test_make_transaction_wallet_does_not_exist(client: TestClient) -> None:
    ...
    assert response.json()["error"]["message"] == f"Wallet with address <{wallet_address}> does not exist."
...
def test_make_transaction_same_wallets(client: TestClient) -> None:
    ...
    assert response.json()["error"][
               "message"] == (f"You are trying to make transaction from wallet with address <{public_key}> to "
...
def test_make_transaction_not_enough_balance(client: TestClient) -> None:
    ...
    assert response.json()["error"]["message"] == f"Wallet with address <{public_key1}> does not have enough balance."
